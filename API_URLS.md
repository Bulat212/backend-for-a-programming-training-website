...
**4.3 /shop/? (GET)**
   ...
   - category=background_profile       фильтрация по названию категории background_profile
   - ordering=price_in_stars           сортировка по звездам, в порядке возвостания
   - ordering=-price_in_stars           сортировка по звездам, в порядке убывания
   - ordering=price_in_coin             сортировка по коинам, в порядке возвостания
   - ordering=-price_in_coin            сортировка по коинам, в порядке убывания
...
**9.2 /delete-comment/<int:comment_id>/ (GET)** 
   - **DELETE**: Удаляет комментарий по переданному comment_id. Админ может удалять любые комментарии, пользователь только свои
   Ничего не выдает, если успешно удалилось
   Если обычный пользователь пытается удалить чужой комментарий:
   {
...
**9.4 /write-comment/ (POST)** авторизован
